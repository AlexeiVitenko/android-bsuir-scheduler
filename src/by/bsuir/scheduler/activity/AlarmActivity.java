...
public class AlarmActivity extends PreferenceActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      //  sharedPref = PreferenceManager.getDefaultSharedPreferences(this);// getSharedPreferences(ALARM_PREF,
        ...
        // ЗАГЛУШКА
        int numberOfLessons = getIntent().getIntExtra(PAIR_NUMBER, 5);
        //

        String[] summaryValues = getResources().getStringArray(R.array.alarm_summary_values);
        ...
        alarmBeforeLesson.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                wasModified = true;
                int value = Integer.parseInt((String) newValue);
    ...
    protected void onPause() {
        ...
        editor.putString(ALARM_LESSON, "" + Integer.parseInt(alarmBeforeLesson.getValue()));
    ...
    private static GregorianCalendar calculateAlarmTime(Context context, Day day) {
        ...
        if (day.getCount() > 0) {
            ...
            // context.getSharedPreferences(ALARM_PREF, MODE_PRIVATE);
            ...
            temp.setTimeInMillis(sharedPref.getLong(ALARM_TIME, System.currentTimeMillis()));
            ...
            if (Integer.parseInt(sharedPref.getString(AlarmActivity.ALARM_TYPE, "" + 0)) == 1) {
                ...
                temptime[2] = temptime[0] - alarm.get(GregorianCalendar.HOUR_OF_DAY);
